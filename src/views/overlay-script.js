 ...
 *
 * Feedback visual de lo que hace el MCP: una flecha en la posición del click,
 * una onda azul y una etiqueta con el elemento tocado (tag#id.class).
 *
 * EXCLUSIVO DE LA IA: el overlay NO escucha eventos del DOM. Si lo hiciera, el
 * mouse físico del usuario y los clicks de la IA serían indistinguibles (ambos
 * llegan con isTrusted:true) y la flecha se "pegaría" al cursor del usuario.
 * En vez de eso, el proxy CDP espía (solo-observación) los Input.dispatchMouseEvent
 * que manda chrome-devtools-mcp — el mouse físico NUNCA viaja por CDP — y llama
 * a `window.__clAiPointer(type, x, y)`, la ÚNICA vía que dibuja el overlay.
 *
...
const OVERLAY_SOURCE = `(() => {
  ...
  let cur = null, pin = null, last = null, styled = false;
  function ensure() {
    ...
    if (!pin) { pin = document.createElement('div'); pin.id = '__cl_pin'; document.body.appendChild(pin); }
    if (!last) { last = document.createElement('div'); last.id = '__cl_last'; last.setAttribute('role', 'status'); document.body.appendChild(last); }
    return true;
  }
  // ÚNICA entrada al overlay: la llama el proxy con los clicks de la IA.
  window.__clAiPointer = function (type, x, y) {
    if (!ensure()) return;
    ...
    l.style.left = x + 'px'; l.style.top = y + 'px'; l.textContent = desc;
    ...
    last.style.left = x + 'px'; last.style.top = y + 'px';
    last.textContent = 'click: ' + desc + ' @' + Math.round(x) + ',' + Math.round(y);
    last.style.display = 'block';
    // Legible por take_snapshot (árbol de accesibilidad), sin depender de píxeles.
    try { document.documentElement.setAttribute('data-cl-lastclick', desc + ' @' + Math.round(x) + ',' + Math.round(y)); } catch (_) {}
  };
  if (!document.body) addEventListener('DOMContentLoaded', ensure);
  else ensure();
})();`;
